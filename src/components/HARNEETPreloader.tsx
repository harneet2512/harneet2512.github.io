...
const HARNEETPreloader: React.FC<HARNEETPreloaderProps> = ({ onComplete }) => {
  ...
  // Strict 5-second fallback timer
  useEffect(() => {
    fallbackTimerRef.current = setTimeout(() => {
      console.log('Fallback timer triggered - forcing completion after 5 seconds')
      if (!isCompleted) {
        setIsCompleted(true)
        setProgress(100)
  ...
  }, [currentLetterIndex, onComplete, isCompleted]);
  ...
  useEffect(() => {
    ...
    const animateLetter = (index: number) => {
      ...
      // Calculate letter position on screen
      const letterSpacing = 176;
      ...
      const animateStylus = () => {
        ...
        if (path.length > 0) {
          ...
          if (currentPoint && nextPoint) {
            const localProgress = pathIndex - Math.floor(pathIndex);
            
            // Smooth interpolation with cubic bezier for organic movement
            const interpolatedX = currentPoint.x + (nextPoint.x - currentPoint.x) * easeInOutCubic(localProgress);
            const interpolatedY = currentPoint.y + (nextPoint.y - currentPoint.y) * easeInOutCubic(localProgress);
            
            // Add subtle organic movement variations
            const timeVariation = Math.sin(elapsed * 0.01) * 0.5;
            const organicX = interpolatedX + timeVariation;
            const organicY = interpolatedY + timeVariation;
            
            const globalX = containerCenterX - 70 + organicX;
            const globalY = containerCenterY - 90 + organicY;
